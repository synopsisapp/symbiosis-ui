...
async function generateIconFiles(
  ...
  const spriteUpToDate = iconNames.every((name: string) =>
    currentSprite.includes(`id=${name}`),
  );
  const typesUpToDate = iconNames.every((name: string) =>
  ...
  const stringifiedIconNames = iconNames.map((name: string) =>
    JSON.stringify(name),
  );
...
async function generateSvgSprite({
  ...
  const symbols = await Promise.all([
    ...
    ...internalFiles.map(async (file) =>
      generateSymbol(file, internalInputDir),
    ),
  ]);
